<template>
  <div class="v-input-wrapper">
    <div class="v-input">
      <div v-show="label" class="v-input__top-label">{{ label }}</div>
      <div
        :class="[
          'v-input__input-wrapper',
          { 'v-input__input-wrapper--errors': errors },
        ]"
      >
        <div class="v-input__errors">{{ errors }}</div>
        <input
          :id="inputId"
          :type="inputType"
          class="v-input__input"
          :value="value"
          :placeholder="placeholder"
          @keydown="$emit('keydown', $event.target.value)"
          @input="$emit('input', $event.target.value)"
        />
        <svg
          v-show="isPasswordInput"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="#D7E1EA"
          xmlns="http://www.w3.org/2000/svg"
          :class="[
            'v-input__input-passwd-icon',
            {
              'v-input__input-passwd-icon--close': showPasswordEye,
            },
          ]"
          @click="handleClickEye"
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M12 4.25C8.30768 4.25 5.62703 5.5736 3.86629 7.1989C2.99041 8.0074 2.34365 8.88909 1.91294 9.71605C1.48923 10.5296 1.25 11.3384 1.25 12C1.25 12.6616 1.48923 13.4704 1.91294 14.284C2.34365 15.1109 2.99041 15.9926 3.86629 16.8011C5.62703 18.4264 8.30768 19.75 12 19.75C15.6923 19.75 18.373 18.4264 20.1337 16.8011C21.0096 15.9926 21.6564 15.1109 22.0871 14.284C22.5108 13.4704 22.75 12.6616 22.75 12C22.75 11.3384 22.5108 10.5296 22.0871 9.71605C21.6564 8.88909 21.0096 8.0074 20.1337 7.1989C18.373 5.5736 15.6923 4.25 12 4.25ZM11.9999 9.75003C10.7572 9.75003 9.74988 10.7574 9.74988 12C9.74988 13.2427 10.7572 14.25 11.9999 14.25C13.2425 14.25 14.2499 13.2427 14.2499 12C14.2499 10.7574 13.2425 9.75003 11.9999 9.75003ZM8.24988 12C8.24988 9.92896 9.92881 8.25003 11.9999 8.25003C14.0709 8.25003 15.7499 9.92896 15.7499 12C15.7499 14.0711 14.0709 15.75 11.9999 15.75C9.92881 15.75 8.24988 14.0711 8.24988 12Z"
          />
        </svg>
      </div>
      <span class="v-input__bottom-placeholder">{{ bottomPlaceholder }}</span>
    </div>
  </div>
</template>

<script>
export default {
  model: {
    prop: 'value',
    event: 'input',
  },
  props: {
    value: {
      type: String,
      default: '',
    },
    label: {
      type: String,
      default: '',
    },
    nativeType: {
      type: String,
      default: 'text',
    },
    bottomPlaceholder: {
      type: String,
      default: '',
    },
    errors: {
      type: String,
      default: '',
    },
    inputId: {
      type: String,
      default: '',
    },
    placeholder: {
      type: String,
      default: '',
    },
  },
  emits: ['input', 'keydown'],
  data() {
    return {
      showPasswordEye: false,
    }
  },
  computed: {
    isPasswordInput() {
      return this.nativeType === 'password'
    },
    inputType() {
      if (this.isPasswordInput && this.showPasswordEye) {
        return 'text'
      }
      return this.nativeType
    },
  },
  methods: {
    /**
     * Click eye event
     */
    handleClickEye() {
      this.showPasswordEye = !this.showPasswordEye
    },
  },
}
</script>

<style lang="scss" scoped>
.v-input {
  position: relative;
  display: flex;
  flex-direction: column;

  &__top-label {
    font-family: 'Inter', sans-serif;
    font-size: 14px;
    line-height: 100%;
    color: #000638;
    margin-bottom: 10px;
  }

  &__input {
    width: 100%;
    height: 100%;
    padding: 4px 8px;
    border: none;
    border-radius: 8px;

    &:focus {
      outline: none;
    }
  }

  &__input-passwd-icon {
    cursor: pointer;

    &--close {
      fill: #000638;
    }
  }

  &__input-wrapper {
    display: flex;
    align-items: center;
    position: relative;
    height: 38px;
    background: #fff;
    border: 1px solid #d7e1ea;
    border-radius: 8px;
    width: 100%;
    padding-right: 12px;

    &--errors {
      border: 1px solid #ef233c;
    }
  }

  &__errors {
    white-space: nowrap;
    font-size: 12px;
    position: absolute;
    color: #ef233c;
    bottom: -20px;
    right: 0;
    z-index: 1;
  }

  &__bottom-placeholder {
    font-family: Inter, sans-serif;
    font-style: normal;
    font-weight: normal;
    font-size: 11px;
    line-height: 100%;
    color: #778699;
    margin-top: 4px;
  }
}
</style>
