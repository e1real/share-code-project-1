  ...
  <div class="page-wrapper">
    <div class="page">
      ...
      <div class="page__page-head page-head">
        <h4 class="page-head__title">Campaigns - {{ form.name }}</h4>
      </div>
      <form
...
export default {
  ...
  async beforeMount() {
    const { data } = await this.$api(`campaigns/${this.$route.params.id}`)
    this.form = data
    this.form.start_date = new Date(data.start_date)
    this.form.end_date = new Date(data.end_date)
  },
  methods: {
    ...
    async submit() {
      ...
      const screenIds = this.form.screens.length
        ? this.form.screens.map((el) => el.screen_id)
        : []
      const screenGroupIds = this.form.screen_groups.length
        ? this.form.screen_groups.map((el) => el.screen_group_id)
        : []
      const placeIds = this.form.places.length
        ? this.form.places.map((el) => el.place_id)
        : []
      ...
      try {
        if (this.isNewRecord) {
          ...
          await this.$api.patch(`campaigns/${this.$route.params.id}`, payload)
        }
    ...
    handleClickItemScreenAutocomplete(value) {
      ...
      this.screenAutocompleteData.splice(findExist, 1)
    },
    /**
     * Remove item in tags
     */
    handleClickRemoveItem({ item, index }) {
      ...
      this.form[item.type].splice(index, 1)
    },
  },
}
</script>
